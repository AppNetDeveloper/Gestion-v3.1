...
public function logoUpdate(UpdateGeneralSettingRequest $request, GeneralSettings $logoSettings)
{
    try {
        ...
        return back()->with(['message' => 'Logo actualizado correctamente.', 'type' => 'success']);
    } catch (QueryException $e) {
        // Registrar la excepción para obtener más detalles sobre el error
        logger()->error('Error al actualizar el logo: ' . $e->getMessage());
        return back()->with(['message' => 'Error al actualizar el logo.', 'type' => 'error']);
    }
