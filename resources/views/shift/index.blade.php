                        ...
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                    @forelse ($shift as $shifts)
                                    <tr class="border border-slate-100 dark:border-slate-900 relative">
                                        <td class="table-td sticky left-0"># {{ $shifts->id }}</td>
                                        <td class="table-td">
                                            <span>{{ __( $shifts->name )}}</span>
                                        </td>
                                        <td class="table-td">{{ $shifts->created_at?->toFormattedDateString() ?? 'Not Available' }}</td>
                                        <td class="table-td">
                                            <div class="action-btns space-x-2 flex">
                                                {{-- show --}}
                                                @can('role show')
                                                <a class="action-btn" href="{{ route('controltime.show', $shifts) }}" x-data="{ tooltip: 'View' }" x-tooltip="tooltip">
                                                    <iconify-icon icon="ph:eye-light"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{-- Edit --}}
                                                @can('role update')
                                                <a class="action-btn" href="{{ route('controltime.edit', ['controltime' => $shifts]) }}">
                                                    <iconify-icon icon="uil:edit"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{-- delete --}}
                                                @can('controltime delete')
                                                <form id="deleteForm{{ $shifts->id }}" method="POST" action="{{ route('controltimes.destroy', $shifts) }}" class="cursor-pointer">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn" onclick="sweetAlertDelete(event, 'deleteForm{{ $shifts->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'controltime.index'" :data="$shift" />
                        </div>
                    </div>
                </div>
            </div>
