<x-app-layout>
        ...
        <div class="card-body p-6">
            ...
            <div class="flex flex-wrap justify-end items-center space-x-3 mb-6">
                @can('projects update')
                    ...
                    @endif
                @endcan
                {{-- Aquí podrías añadir más botones específicos del proyecto --}}
            </div>
            ...
            <div class="overflow-x-auto">
                <table id="projectTasksTable" class="w-full border-collapse dataTable">
                    <thead class="bg-slate-100 dark:bg-slate-700">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase tracking-wider border-b-2 border-slate-200 dark:border-slate-600">{{ __('Title') }}</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase tracking-wider border-b-2 border-slate-200 dark:border-slate-600">{{ __('Assigned To') }}</th>
    ...
    @push('styles')
        ...
        <style>
            ...
            /* ... (otros estilos necesarios como los de DataTables pagination, SweetAlert, etc.) ... */
        </style>
    @endpush
        ...
        <script>
            $(function() {
                ...
                const projectTasksDataTable = $('#projectTasksTable').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: '{{ route("projects.tasks.data", $project->id) }}', // Ruta para obtener datos de tareas del proyecto actual
                        type: 'GET',
                    ...
                    columns: [
                        ...
                        { data: 'assigned_users_list', name: 'users.name', className: 'table-td', orderable: false, searchable: false }, // No se puede ordenar/buscar fácilmente por lista concatenada
                ...
                $('#projectTasksTable').on('click', '.deleteTask', function () {
                    ...
                    const projectId = $(this).data('project-id'); // Asegúrate de que este data-attribute se añade en TaskController@data
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            // La ruta para destroy de tareas es shallow: /tasks/{task}
                            fetch(`/tasks/${taskId}`, {
                                method: 'DELETE',
                                headers: {
