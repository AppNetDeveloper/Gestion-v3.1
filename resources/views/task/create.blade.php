<x-app-layout>
    <div class="mb-6">
        {{-- Breadcrumb --}}
        {{-- $project y $breadcrumbItems se pasan desde TaskController@create --}}
        <x-breadcrumb :breadcrumb-items="$breadcrumbItems ?? []" :page-title="__('Create New Task for Project: :projectTitle', ['projectTitle' => $project->project_title])" />
    </div>
    ...
    <div class="card bg-white dark:bg-slate-800 shadow-xl rounded-lg">
        <div class="card-body p-6">
            <h3 class="text-xl font-semibold text-slate-700 dark:text-slate-200 mb-6">{{ __('Task Details') }}</h3>

            {{-- La ruta para store de tareas anidadas es projects.tasks.store --}}
            <form action="{{ route('projects.tasks.store', $project->id) }}" method="POST" id="taskForm">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" id="estimated_hours" name="estimated_hours" step="0.1" min="0" value="{{ old('estimated_hours') }}"
                    ...
                    {{-- Usuarios Asignados --}}
                    <div class="lg:col-span-3 md:col-span-2"> {{-- Ocupar más espacio --}}
                        ...
                        <select id="assigned_users" name="assigned_users[]" class="inputField select2 w-full" multiple="multiple">
                            {{-- $assignableUsers se pasa desde el TaskController@create --}}
                            @foreach($assignableUsers ?? [] as $id => $name)
                                <option value="{{ $id }}" {{ (collect(old('assigned_users'))->contains($id)) ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                ...
                if (typeof $.fn.select2 !== 'undefined') {
                    $('.select2').each(function() {
                        ...
                        if ($select.prop('multiple')) {
                            options.placeholder = "{{ __('Select users...') }}"; // Placeholder para multi-select
                        }
