...
<x-app-layout>
    ...
    <style>
        ...
        #mapControls select, #mapControls input[type="date"] { padding: 0.3rem 0.5rem; border-radius: 0.25rem; border: 1px solid #ccc; font-size: 0.875rem; }
        ...
        body.history-mode #historyControls { display: flex; }
    ...
    {{-- Plugin Polyline Decorator JS (Lo dejamos por si se arregla en futuro) --}}
    ...
    <script type="module">
        document.addEventListener('DOMContentLoaded', function () {
            ...
            setTimeout(() => { /* ... código igual que antes ... */
            ...
            let historyLayerGroup = L.layerGroup().addTo(map); // Grupo para capas de historial
            ...
            // Limpiar capas de historial (polyline y marcadores/tooltips de hora)
            ...
            // --- Obtener y dibujar datos de HISTORIAL (con etiquetas de hora MUESTREADAS) ---
            async function fetchAndDrawHistory(userId, dateString) {
                 ...
                 try {
                    ...
                    if (!historyData || historyData.length === 0) { mapStatusText.textContent = `Modo: Historial - No se encontraron datos para ${knownUsers[userId] || `Usuario ${userId}`} el ${dateString}.`; console.log("No se encontraron datos de historial."); return; }

                    const latLngs = [];
                    const historyPointSamplingRate = 20; // <--- Mostrar marcador/hora cada 20 puntos
                    let pointsDrawn = 0;

                    historyData.forEach((point, index) => { // <--- Añadir index
                        const lat = parseFloat(point.latitude);
                        const lon = parseFloat(point.longitude);
                        if (!isNaN(lat) && !isNaN(lon)) {
                            latLngs.push([lat, lon]); // La polyline siempre usa todos los puntos

                            // --- MODIFICACIÓN: Dibujar marcador/tooltip solo cada N puntos ---
                            if (index % historyPointSamplingRate === 0) {
                                let pointTimeText = '';
                                if (point.recorded_at) {
                                    try {
                                        const dateStr = point.recorded_at.endsWith('Z') ? point.recorded_at : point.recorded_at + 'Z';
                                        const recordedAt = new Date(dateStr);
                                        if (!isNaN(recordedAt)) {
                                             pointTimeText = recordedAt.toLocaleTimeString('es-ES', { hour: '2-digit', minute:'2-digit' }); // Formato HH:MM
                                        }
                                    } catch (e) { console.error("Error parseando fecha para tooltip:", point.recorded_at, e); }
                                }

                                if (pointTimeText) { // Solo añadir si tenemos hora
                                    L.circleMarker([lat, lon], {
                                        radius: 3, color: '#888', weight: 1, fillColor: '#aaa', fillOpacity: 0.5
                                    })
                                    .bindTooltip(pointTimeText, {
                                        permanent: true, direction: 'top', offset: [0, -6], className: 'history-time-tooltip'
                                     })
                                    .addTo(historyLayerGroup);
                                    pointsDrawn++; // Incrementar contador
                                }
                            }
                            // --- FIN MODIFICACIÓN ---
                        }
                    ...
                    if (latLngs.length > 1) {
                        ...
                        // Log modificado para mostrar cuántos marcadores se dibujaron
                        console.log(`Historial dibujado con ${latLngs.length} puntos en la línea y ${pointsDrawn} marcadores/horas mostrados.`);
                        ...
                        // El circleMarker ya se habrá añadido (si index era 0)
            ...
            setTimeout(() => {
                ...
                updateLiveData().finally(() => { if (currentMode === 'live' && !liveUpdateIntervalId) { startLiveUpdates(); } });
            }, 150);
