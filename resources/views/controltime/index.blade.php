                        ...
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                    @forelse ($timeControlStatus as $controlStatus)
                                    <tr class="border border-slate-100 dark:border-slate-900 relative">
                                        <td class="table-td sticky left-0"># {{ $controlStatus->id }}</td>
                                        <td class="table-td">
                                            <span>{{ __( $controlStatus->table_name )}}</span>
                                        </td>
                                        <td class="table-td">{{ $controlStatus->created_at?->toFormattedDateString() ?? 'Not Available' }}</td>
                                        <td class="table-td">
                                            <div class="action-btns space-x-2 flex">
                                                {{-- show --}}
                                                @can('role show')
                                                <a class="action-btn" href="{{ route('controltime.show', $controlStatus) }}" x-data="{ tooltip: 'View' }" x-tooltip="tooltip">
                                                    <iconify-icon icon="ph:eye-light"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{-- Edit --}}
                                                @can('role update')
                                                <a class="action-btn" href="{{ route('controltime.edit', ['controltime' => $controlStatus]) }}">
                                                    <iconify-icon icon="uil:edit"></iconify-icon>
                                                </a>
                                                @endcan
                                                {{-- delete --}}
                                                @can('controltime delete')
                                                <form id="deleteForm{{ $controlStatus->id }}" method="POST" action="{{ route('controltimes.destroy', $controlStatus) }}" class="cursor-pointer">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn" onclick="sweetAlertDelete(event, 'deleteForm{{ $controlStatus->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'controltime.index'" :data="$timeControlStatus" />
                        </div>
                    </div>
                </div>
            </div>
